        ...
        if (!isRefresh) {
            ...
            this.dtablehistory = $('#d-table-history').dataTable($.extend({}, $.fn.dataTable.defaults, {
                ...
                "aoColumns": [
                    ...
                    {"mData": function(row, type){if(type == "display") return countlyCommon.formatTimeAgo(row.ts); else return row.ts;}, "sType":"format-ago", "sTitle": jQuery.i18n.map["common.time"] }
                ],
                "fnInitComplete": function(oSettings, json) {
                    $.fn.dataTable.defaults.fnInitComplete(oSettings, json);
                    var tableWrapper = $("#" + oSettings.sTableId + "_wrapper");
                ...
                if(id){
                    if(el.hasClass("view-history")){
                        ...
                        countlyAppUsers.exportUser(JSON.stringify({uid:id}),function(error,export_id,task_id){
                            ...
                            {
                                CountlyHelpers.notify({type:"ok",title:jQuery.i18n.map["common.success"], message:jQuery.i18n.map["app-users.export-finished"], info:jQuery.i18n.map["consent.export-finished-click"], sticky:false,clearAll:true,onClick:function(){
