window.ActivitiesView = countlyView.extend({
    convertFilter: {
    ...
    beforeRender: function() {
		...
		if(this.template)
			return $.when(countlyActivities.initialize()).then(function () {});
		else{
			var self = this;
			return $.when($.get(countlyGlobal["path"]+'/activity/templates/crashes.html', function(src){
				self.template = Handlebars.compile(src);
			}), countlyActivities.initialize()).then(function () {});
		}
    },
    processData:function(){
        var self = this;
        var crashData = countlyActivities.getData();
        this.dtable = $('#crash-table').dataTable($.extend({}, $.fn.dataTable.defaults, {
            "aaSorting": [[ 5, "desc" ]],
            "bServerSide": true,
            "sAjaxSource": countlyCommon.API_PARTS.data.r + "?api_key="+countlyGlobal.member.api_key+"&app_id="+countlyCommon.ACTIVE_APP_ID+"&method=activities",
            ...
            if(val != ""){
                ...
                if(val === "crash-resolve"){
                    CountlyHelpers.confirm(jQuery.i18n.prop("crashes.confirm-action-resolved", self.selectedCrashesIds.length), "red", function (result) {
                        ...
                        countlyActivities.markResolve(self.selectedCrashesIds, function(data){
                            if(!data){
                    ...
                    CountlyHelpers.confirm(jQuery.i18n.prop("crashes.confirm-action-unresolved", self.selectedCrashesIds.length), "red", function (result) {
                        ...
                        countlyActivities.markUnresolve(self.selectedCrashesIds, function(data){
                            if(!data){
                                CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
                            }
                            else{
                    ...
                    CountlyHelpers.confirm(jQuery.i18n.prop("crashes.confirm-action-hide", self.selectedCrashesIds.length), "red", function (result) {
                        ...
                        countlyActivities.hide(self.selectedCrashesIds, function(data){
                            if(!data){
                                CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
                            }
                            else{
                    ...
                    CountlyHelpers.confirm(jQuery.i18n.prop("crashes.confirm-action-resolving", self.selectedCrashesIds.length), "red", function (result) {
                        ...
                        countlyActivities.resolving(self.selectedCrashesIds, function(data){
                            if(!data){
                                CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
                            }
                            else{
                    ...
                    CountlyHelpers.confirm(jQuery.i18n.prop("crashes.confirm-action-delete", self.selectedCrashesIds.length), "red", function (result) {
                        ...
                        countlyActivities.del(self.selectedCrashesIds, function(data){
                            if(!data){
                                CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
                            }
                            else{
    ...
    refresh:function () {
        ...
        if(this.loaded){
            this.loaded = false;
            $.when(countlyActivities.refresh()).then(function () {
                self.loaded = true;
                ...
                var chartData = countlyActivities.getChartData(self.curMetric, self.metrics[self.curMetric]);
                countlyCommon.drawTimeGraph(chartData.chartDP, "#dashboard-graph");
                //app.localize();
            });
        }
	...
	getExportQuery: function(){
        ...
        var qstring = {
            ...
            db: "countly",
            collection: "app_activitygroups"+countlyCommon.ACTIVE_APP_ID,
            query:this._query || {}
        };
    ...
    loadAndRefresh: function() {
        ...
        this._query = filter;
        app.navigate("/activities/filter/"+JSON.stringify(filter), false);
        this.dtable.fnPageChange(0);
        this.refresh(true);
    }
});

window.ActivitygroupView = countlyView.extend({
	initialize:function () {
        this.loaded = true;
    },
    beforeRender: function() {
        countlyActivities.reset();
		if(this.template)
			return $.when(countlyActivities.initialize(this.id)).then(function () {});
		else{
			var self = this;
			return $.when($.get(countlyGlobal["path"]+'/activity/templates/crashgroup.html', function(src){
				self.template = Handlebars.compile(src);
			}), countlyActivities.initialize(this.id)).then(function () {});
		}
    },
    renderCommon:function (isRefresh) {
		var url = location.protocol+'//'+location.hostname+(location.port ? ':'+location.port: '')+countlyGlobal["path"]+"/crash/";
        var crashData = countlyActivities.getGroupData();
        if(crashData.url)
               url += crashData.url;
		...
		if (!isRefresh) {
			this.metrics = countlyActivities.getMetrics();
        ...
        if(countlyGlobal["apps"][countlyCommon.ACTIVE_APP_ID].type != "web"){
            this.templateData["ranges"]=[
                ...
                },
                {
                    ...
                    // "avg":countlyCommon.timeString((crashData.run.total/crashData.run.count)/1000).toFixed(1)
                    "min":(crashData.run.min/1000).toFixed(1)+"秒",
                    "max":(crashData.run.max/1000).toFixed(1)+"秒",
                    "avg":((crashData.run.total/crashData.run.count)/1000).toFixed(1)+"秒"
                }
            ];
            ...
            this.templateData["bars"]=[
                {
                    ...
                    "data": countlyActivities.getBoolBars("root"),
        ...
        if (!isRefresh) {
            ...
            $('.crash-share input[type=checkbox]').change(function(){
                ...
                countlyActivities.modifyShare(crashData._id, opts);
            });
            
            $('#crash-share-public').change(function(){
                if($(this).is(":checked")) {
                    countlyActivities.share(crashData._id, function(data){
                        if(data)
                            $(".crash-share").show();
                        else
                            CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
                    ...
                    countlyActivities.unshare(crashData._id, function(data){
                        if(data)
                            $(".crash-share").hide();
                        else
                            CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red");
            ...
            $("#add_comment").click(function(){
                ...
                countlyActivities.addComment(crashData._id, comment, function(data){
                    if(data){
                        self.refresh();
                        $("#comment").val("");
                    }
            ...
            $("#notes").on("click", ".crash-comment-edit", function(){
                ...
                if(!container.find("#comment_edit").length){
                    ...
                    container.find(".edit_comment").click(function(){
                        ...
                        countlyActivities.editComment(crashData._id, comment, function(data){
                            if(data){
                                self.refresh();
                                container.find("#comment_edit").remove();
                                container.find(".text").show();
            ...
            $("#notes").on("click", ".crash-comment-delete", function(){
                ...
                CountlyHelpers.confirm(jQuery.i18n.map["crashes.confirm-comment-delete"], "red", function (result) {
                    ...
                    countlyActivities.deleteComment(crashData._id, ob, function(data){
                        if(data){
        ...
        $(".crash-manipulate-options").on("cly-select-change", function (e, val) {
            if(val != ""){
                ...
                if(val === "crash-resolve"){
                    countlyActivities.markResolve(crashData._id, function(version){
                        if(version){
                    ...
                    countlyActivities.resolving([crashData._id], function(data){
                        if(!data){
                            CountlyHelpers.alert(jQuery.i18n.map["crashes.try-later"], "red"); 
                        }
                        else{
                    ...
                    countlyActivities.markUnresolve(crashData._id, function(data){ 
                        if(data){
                    ...
                    countlyActivities.hide(crashData._id, function(data){
                        if(data){
                    ...
                    countlyActivities.show(crashData._id, function(data){
                        if(data){
                    ...
                    CountlyHelpers.confirm(jQuery.i18n.map["crashes.confirm-delete"], "red", function (result) {
                        ...
                        countlyActivities.del(crashData._id, function (data) {
                            if(data){
    ...
    refresh:function () {
        ...
        if(this.loaded){
            this.loaded = false;
            $.when(countlyActivities.initialize(this.id, true)).then(function () {
                self.loaded = true;
                if (app.activeView != self) {
                    return false;
                }
...
//register views
app.activitiesView = new ActivitiesView();
app.activitygroupView = new ActivitygroupView();

app.route('/activities', 'activities', function () {
    this.activitiesView._filter = false;
    this.activitiesView._query = null;
	this.renderWhenReady(this.activitiesView);
});

app.route('/activities/filter/*query', 'userdata', function (query) {
    try{
    ...
    this.activitiesView._query = query;
    this.activitiesView._filter = true;
	this.renderWhenReady(this.activitiesView);
});

app.route('/activities/:group', 'crashgroup', function (group) {
	this.activitygroupView.id = group;
    this.renderWhenReady(this.activitygroupView);
});
...
app.addPageScript("/users/#", function(){
    if(app.activeView && app.activeView.tabs){
        ...
        app.activeView.dtablecrashes = $('#d-table-crashes').dataTable($.extend({}, $.fn.dataTable.defaults, {
            ...
            "bServerSide": true,
            "bFilter": false,
            "sAjaxSource": countlyCommon.API_PARTS.data.r + "?api_key="+countlyGlobal.member.api_key+"&app_id="+countlyCommon.ACTIVE_APP_ID+"&method=user_activities&uid="+userDetails.uid,
            "fnServerData": function ( sSource, aoData, fnCallback ) {
                self.request = $.ajax({
                    "dataType": 'json',
                    "type": "POST",
...
$( document ).ready(function() {
    ...
    app.addAppSwitchCallback(function(appId){
        if(app._isFirstLoad!=true)
            countlyActivities.loadList(appId);
    });
    if(!production){
        CountlyHelpers.loadJS("activity/javascripts/marked.min.js");
    }
